...
NSString * const kPublicKey = @"kPublicKey";
...
+ (NSString *)EJ_RSAEncryptString:(NSString *)str {
    NSData *data = [RSA EJ_RSAEncryptData:[str dataUsingEncoding:NSUTF8StringEncoding]];
...
+ (NSData *)EJ_RSAEncryptData:(NSData *)data {
    ...
    SecKeyRef keyRef = [RSA addPublicKey:kPublicKey];
    if(!keyRef) {
        return nil;
    }
    ...
    NSData *ret = nil;
    if (status != 0) {
        //NEJog(@"SecKeyEncrypt fail. Error Code: %ld", status);
    }else{
...
+ (NSString *)EJ_RSADecryptString:(NSString *)str {
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
    data = [RSA EJ_RSADecryptData:data];
    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    return ret;
}
...
+ (NSString *)EJ_RSAEncryptString:(NSString *)str publicKey:(NSString *)pubKey{
    NSData *data = [RSA EJ_RSAEncryptData:[str dataUsingEncoding:NSUTF8StringEncoding] publicKey:pubKey];
    NSString *ret = base64_encode_data(data);
    return ret;
}
...
+ (NSString *)EJ_RSADecryptString:(NSString *)str publicKey:(NSString *)pubKey{
    NSData *data = [[NSData alloc] initWithBase64EncodedString:str options:NSDataBase64DecodingIgnoreUnknownCharacters];
    data = [RSA EJ_RSADecryptData:data publicKey:pubKey];
    NSString *ret = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    return ret;
}
